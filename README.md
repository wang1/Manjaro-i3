...
49. vscode进行远程开发，一直出现`Remote host Extension Terminated`错误，然后必须reload window，但一会又出现了，查了许多资料，基本都给出的是扩展有冲突，依次禁用再启用来排除，或者其它情况。排查许久，发现是`.ssh/known_host`文件有过时的一些主机信息，删除该文件后一切正常！
